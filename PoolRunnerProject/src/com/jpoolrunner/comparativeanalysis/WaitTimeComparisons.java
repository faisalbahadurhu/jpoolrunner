...
public class WaitTimeComparisons extends JPanel {
	DefaultCategoryDataset dataset = new DefaultCategoryDataset();
	
	/**
	 * Create the panel.
	 */
	public WaitTimeComparisons() {

	}

	public WaitTimeComparisons(Vector<String> tpsAndTestNames, int width,int height,String[] workLoadForBins,String fiftyNinty) {
		...
		for(int i=0;i<tpsAndTestNames.size();i++){
			...
			for(int n=0;n<workLoadForBins.length;n++){
				...
				if(fiftyNinty.equals("fiftyWt"))
					bins.add(s2Split[1]);
				else if(fiftyNinty.equals("nintyWt"))	 bins.add(s2Split[2]); // for 90 percentile
    ...
    //	Number startZ[][] = {s1 ,s2}; 
     ...
     //   Number endZ[][] = { e1,e2};
		...
		String embed="";
		if(fiftyNinty.equals("fiftyWt")) embed+="50th";
		else if(fiftyNinty.equals("nintyWt")) embed+="90th";
		
		chart.setTitle(fontSetting.getTextTitle(embed+"Percentile Wait Time "));
