...
public class ThroughputSeriesComparisons extends JPanel {
	...
	public ThroughputSeriesComparisons( Vector<String> tpsNTestNames, int width,int height) {
...
//	Vector vectorOfSeries=new Vector(tpsNTestNames.size());
	...
	//	vectorOfVectors.add(throughputPanelSeries);
		...
		if(series.getMinY()<minimum) minimum=series.getMinY();
		if(series.getMaxY()>maximum) maximum=series.getMaxY();
		try{
    		SwingUtilities.invokeLater(new Runnable() {//it is used to tackle exceptions that are generated often see bookmarks JfreeChartException
    			public void run() {
    			//	dataset.removeAllSeries();
 ...
 chart = createChart(dataset,"Throughput Comparison","Time(sec)","Responses/Sec");
	//chart.removeLegend();
 chartPanel = new ChartPanel(chart);
	chartPanel.setPreferredSize(new Dimension(width,height));//width height
